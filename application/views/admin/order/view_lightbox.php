	...
	<div class="modal-body">
		<div class="row">
			...
			<?php if(is_array($vectors))foreach($vectors as $key=>$vector){?>
				<?php if(count($vector) != 0){ $productdata = true; ?>
					<div class="col-sm-12">
						<fieldset>
				...
				<?php } ?>
			<?php }else {$error = '<div class="col-sm-12"><div class="alert alert-danger" role="alert">'.lang('orders_admin_detail_view_not_found_msg').'</div></div>'; } ?>
			<?php }else{ $error = '<div class="col-sm-12"><div class="alert alert-danger" role="alert">'.lang('orders_admin_detail_view_not_found_msg').'</div></div>';} ?>
			<?php 
				if($error != '')
				{
					echo $error;
				}elseif(empty($productdata) && $error == '')
				{
					echo '<div class="col-sm-12"><div class="alert alert-danger" role="alert">'.lang('orders_admin_detail_view_design_not_found_msg').'</div></div>';
				}
			?>
		</div>
	</div>
...
function downloadPNG(url)
{
	...
	$.ajax({
		...
		image.onload = function() {
			...
			if (jQuery('#download-file-print').length > 0)
			{
				var elm = jQuery('#download-file-print');
				var a = elm[0];
			...
			else
			{
				...
				a.setAttribute('id', 'download-file-print');
				//a.innerHTML = 'download';
				a.href = canvas.toDataURL('image/png');
				jQuery('body').append(a);
			}
			setTimeout(function(){
				document.getElementById('download-file-print').click();
				$('body').css('opacity', 1);
			}, 2000);
		}
