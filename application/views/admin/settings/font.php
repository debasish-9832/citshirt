	...
	<tbody>
		...
		<tr>
			<td class="center">
				<div class="checkbox-table">
					<label>
						<input type="checkbox" name="checkb[]" class="checkb" value="<?php echo $font->id; ?>">
					</label>
				</div>
			</td>
			<td>
				<?php 
					if ($font->type == '')
					{
						echo '<a href="'.site_url().'admin/settings/fonts/edit_fonts/'.$font->id.'">'.$font->title.'</a>';
					}else
					{
						echo $font->title;
					}
			...
			<td class="center">
				<?php if ($font->published == 1) { ?>					   
					<a class="btn btn-success btn-xs tooltips action" data-original-title="<?php echo lang('click_unpublish');?>" data-placement="top" rel="unpublish" data-id="<?php echo $font->id; ?>" data-flag="1"><?php echo lang('publish'); ?></a>
				<?php } else { ?>
