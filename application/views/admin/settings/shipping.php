...
<script type="text/javascript">
	...
	var ples = '<?php echo lang('check_box_mgs');?>';
</script>
			...
			<tbody>
				<?php if (is_array($shipping)) foreach ($shipping as $ship) { ?>
						<tr>
								...
								<div class="checkbox-table">
									<label>
										<?php if($ship->default == 1){?>
											<input type="checkbox" class="checkb" disabled="true" value="">
										<?php }else{ ?>
											<input type="checkbox" name="checkb[]" class="checkb" value="<?php echo $ship->id; ?>">
										<?php } ?>										
									</label>
								</div>
							</td>
							...
							<td class="center">
								<?php if($ship->default == 1){?>
									<a class="btn btn-default btn-xs" href="javascript:void(0);"><?php echo lang('publish'); ?></a>
								<?php } else if ($ship->published == 1) { ?>					   
									<a href="javascript:void(0);" class="btn btn-success btn-xs tooltips action" data-original-title="<?php echo lang('click_unpublish');?>" data-placement="top" rel="unpublish" data-id="<?php echo $ship->id; ?>" data-flag="1"><?php echo lang('publish'); ?></a>
								<?php } else { ?>
									<a href="javascript:void(0);" class="btn btn-danger btn-xs tooltips action" data-original-title="<?php echo lang('click_publish');?>" data-placement="top" rel="publish" data-id="<?php echo $ship->id; ?>" data-flag="0"><?php echo lang('unpublish'); ?></a>
								<?php } ?>
							</td>
							<td class="center">
								<div class="visible-md visible-lg hidden-sm hidden-xs">
									<a href="javascript:void(0);" rel="edit" class="btn btn-teal tooltips" data-placement="top" data-original-title="<?php echo lang('edit');?>" onclick="UIModals.init('<?php echo site_url(); ?>admin/settings/edit/shipping/<?php echo $ship->id; ?>')">
