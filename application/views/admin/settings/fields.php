...
<div class="panel panel-default">
	...
	<div class="panel-body" id="panelbody">
		<table id="sample-table-1" class="table table-bordered table-hover">
			...
			<th class="center"><?php echo lang('field_admin_name_attr'); ?></th>
			<th class="center"><?php echo lang('type'); ?></th>
			<th class="center"><?php echo lang('layout'); ?></th>
			<th class="center" style="width: 70px;"><?php echo lang('order'); ?></th>
			...
			<tbody>
				<?php foreach ($fields as $field) { ?>
						<tr>
							...
							<td class="center click_edit" data-label="<?php echo $field->id; ?>"><?php echo $field->order; ?></td>
	...
	function save_order(e)
	{
		...
		if($.isNumeric(value) && (value % 1 == 0) && value >= 0)
		{
			jQuery.ajax({
				...
				data: {field_id: id, order: value},
				dataType: 'html',
				success: function(data){
					if(data != '')
					{
						alert('<?php echo lang('settings_admin_field_save_order_success_msg');?>');
					}else
					{
						alert('<?php echo lang('settings_admin_field_save_order_error_msg');?>');
					}
				}
			});
		}
