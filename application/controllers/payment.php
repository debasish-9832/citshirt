...
class Payment extends Frontend_Controller
{
	...
	{	
		...
		if($this->input->post('payment'))
		{
			...
			$html .= '<td style="border: 1px solid #ccc; padding: 5px;">'.language("print_price", $lang).'</td>';
			...
			$html .= '<td style="border: 1px solid #ccc; padding: 5px;">'.language("option", $lang).'</td>';
			...
			<tr>';
			$total = $order['total'];
			$html .= '<td colspan="8" style="border: 1px solid #ccc; text-align: right;">'.language("total", $lang).':</td>
	...
	function confirm()
	{
		...
		if((isset($user['id']) && $user['id'] != '') && ($this->session->flashdata('msg') != '' || $this->session->flashdata('error') != ''))
		{
