...
class Settings extends Admin_Controller {
    ...
    function index() {
		...
		//currencies.
		$this->data['currencies'] = $this->settings_m->getCurrencies(true);
	...
	function editLang()
	{
		...
		{
			$data = json_decode($this->input->post('language', true), true);
			...
			{
				echo lang('settings_lang_update_data_error_msg');
			}
		}else
		{
	...
	{
		$path = ROOTPATH.DS.'application'.DS.'language'.DS.'english'.DS.'lang.ini';
		if(file_exists($path))
		{
			if(unlink($path))
				$this->session->set_flashdata('msg', lang('settings_lang_save_language_default_success_msg'));
		}
		redirect('admin/settings');
	}
    ...
    function publish($type = '') {
		...
		if($this->input->post('check_all') == 'on')
		{
			$errors['error'] = 0;
			$errors['msg'] = lang('check_box_mgs');
			echo json_encode($errors);
			return;
		}
    }
	...
	function info() 
	{
		 $this->data['breadcrumb'] = lang('settings_admin_info_breadcrumb');
         $this->data['meta_title'] = lang('settings_admin_info_meta_title');
         $this->data['sub_title'] = lang('settings_admin_info_sub_title');
		 
		 $this->data['subview'] = 'admin/settings/info';
		 $this->load->view('admin/_layout_main', $this->data);
	}
}
