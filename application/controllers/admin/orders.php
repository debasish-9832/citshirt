...
class Orders extends Admin_Controller
{
	...
	function index()
	{
		...
		{		
			...
			$this->session->set_userdata('option_order', $this->input->post('option'));
		}
		...
		$orders = $this->order_m->getOrders(false, $config['per_page'], $this->uri->segment(4), $this->session->userdata('search_order'), $this->session->userdata('option_order'));
					...
					{						
						...
						if ($images->length  > 0) {
							foreach ($images as $image) {
								$width 	= $image->getAttribute('width') * 10;
								$image->setAttribute('width', $width);
								
								$height 	= $image->getAttribute('height') * 10;
								$image->setAttribute('height', $height);
								
								$x 	= $image->getAttribute('x') * 10;
								$image->setAttribute('x', $x);
								
								$y 	= $image->getAttribute('y') * 10;
	...
	{		
		...
		// save note.
		if($note = $this->input->post('note'))
		{
			$content = array(
				$order->order_number=>$note
			);
		...
		if($this->input->post('id'))
		{
			...
			if($this->order_m->save($data_his))
			{
				...
				if(count($customer) > 0)
				{
					...
					$message = '<p>Hello, '.$customer->username.'</p><p>The status of product <a target="_blank" href="'.site_url('product/'.$item->product_id).'">'.$item_name.'</a> in your order number '.$customer->order_number.' is changed to '.$status.'.</p><p>Regards,</p><p><a href="'.site_url().'">'.site_url().'</a></p>';
