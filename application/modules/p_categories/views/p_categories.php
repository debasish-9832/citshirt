...
if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	if(count($categories))
	{
		...
		if(isset($content->layout) && $content->layout == 'list')
		{
			...
			$categories = dispayListCate($categories, $cate_id, 'categories/', $show_level, $category->id);
	...
	<script type="text/javascript">
		...
		function list_cate_toggle_<?php echo $category->id;?>(e)
		{
			...
			jQuery('.<?php echo 'module-categories-'.$category->id;?>').children('li').removeClass('actived');
