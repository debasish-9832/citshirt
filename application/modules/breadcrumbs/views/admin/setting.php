...
<form name="setting" class="setting-save" method="POST" action="<?php echo site_url('breadcrumbs/admin/setting/save/'.$id); ?>">
<div class="tabpanel" role="tabpanel">
	...
	<ul class="nav nav-tabs" role="tablist">
		...
		<li class="button-back pull-right"><a href="javascript:void(0)" onclick="grid.module.setting('breadcrumbs')" title="Back to list"><i class="clip-arrow-left-2"></i></a></li>    
	</ul>
	...
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane active" id="tab-general">	
			<div class="form-group">
				...
				<input type="text" class="form-control input-sm validate required" name="title" placeholder="<?php echo lang('title');?>" value="<?php echo $breadcrumbs->title; ?>" data-minlength="2" data-maxlength="200" data-msg="<?php echo lang('breadcrumbs_admin_setting_title_validate');?>"> 
			</div>
			
			<?php
				$content = json_decode($breadcrumbs->content);
			?>
			
			<div class="form-group">
				<label><?php echo lang('breadcrumbs_admin_setting_show_home_icon_title');?></label>
				<div class="row">
					<div class="col-md-4">
						<?php
							if(!isset($content->show_icon))
								$content->show_icon = '';
							$option = array(
								'yes'=>lang('yes'),
								'no'=>lang('no'),
							);
							echo form_dropdown('data[show_icon]', $option, $content->show_icon, 'class="form-control input-sm"');
						?>
					</div>
				</div>
			</div>
			...
			<div class="form-group">
				<label><?php echo lang('breadcrumbs_admin_setting_choose_layout_title');?></label>
				<div class="row">
					<div class="col-md-4">
						<?php
							if(!isset($content->layout))
								$content->layout = '';
							$option = array(
								''=>lang('breadcrumb_default'),
								'breadcrumb-arrow'=>lang('breadcrumbs_arrows'),
							);
							echo form_dropdown('data[layout]', $option, $content->layout, 'class="form-control input-sm"');
						?>
					</div>
				</div>
			</div>
			...
			<div class="form-group">
				<label><?php echo lang('breadcrumbs_admin_setting_class_sfx_title');?></label>
				<input type="text" name="data[class_sfx]" class="form-control input-sm" value="<?php if(isset($content->class_sfx)) echo $content->class_sfx; ?>" placeholder="<?php echo lang('breadcrumbs_admin_setting_class_sfx_title');?>"/>
			</div>
		</div>
		
		<!-- design options -->
		<?php 
			$params = json_decode($breadcrumbs->params, true);
		?>
