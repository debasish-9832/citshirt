...
<form name="setting" class="setting-save" method="POST" action="<?php echo site_url('m_search/admin/setting/save/'.$id); ?>">
<div class="tabpanel" role="tabpanel">
	...
	<ul class="nav nav-tabs" role="tablist">
		...
		<li class="button-back pull-right"><a href="javascript:void(0)" onclick="grid.module.setting('m_search')" title="Back to list"><i class="clip-arrow-left-2"></i></a></li>    
	</ul>
	...
	<div class="tab-content">
		<div role="tabpanel" class="tab-pane active" id="tab-general">	
			<div class="form-group">
				...
				<input type="text" class="form-control input-sm validate required" name="title" placeholder="<?php echo lang('title');?>" value="<?php echo $search->title; ?>" data-minlength="2" data-maxlength="200" data-msg="<?php echo lang('search_admin_setting_title_validate');?>"> 
			</div>
			...
			<div class="form-group">
				<div class="row">
					<div class="col-sm-6">
						<label><?php echo lang('search_admin_setting_search_size_title');?></label>
						<?php
							$size = array(
								''=>lang('search_admin_setting_default_title'),
								'input-lg'=>lang('search_admin_setting_larger_title'),
								'input-sm'=>lang('search_admin_setting_smaill_title'),
							);
							if(isset($options->size))
								$default = $options->size;
							else	
								$default = '';
							echo form_dropdown('options[size]', $size, $default, 'class="form-control input-sm"');
						?>
					</div>
					<div class="col-sm-6">
						<label><?php echo lang('search_admin_setting_display_title');?></label>
						<?php
							$display = array(
								''=>lang('search_admin_setting_display_default_title'),
								'input'=>lang('search_admin_setting_display_input_title'),
								'icon'=>lang('search_admin_setting_display_icon_title'),
							);
							if(isset($options->display))
								$default = $options->display;
							else	
								$default = '';
							echo form_dropdown('options[display]', $display, $default, 'class="form-control input-sm"');
						?>
					</div>
				</div>
			</div>
			...
			<div class="form-group">
				<label><?php echo lang('search_admin_setting_class_sfx_title');?></label>
				<input type="text" name="options[class_sfx]" class="form-control input-sm" value="<?php if(isset($options->class_sfx)) echo $options->class_sfx; ?>" placeholder="<?php echo lang('search_admin_setting_class_sfx_title');?>"/>
			</div>
		</div>
