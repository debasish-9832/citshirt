...
<?php echo $css; ?>
<ul id="bx-slider-<?php echo $slider->id; ?>" style="padding: 0;">
	<?php 
			...
			if(isset($data['display'][$key]) && $data['display'][$key] == 'image') // display image.
			{
				if($data['url'][$key] != '') // show url
				{
					if($data['thumbnail'][$key] == 'show')
						echo '<li style="left: 0px;"><a href="'.base_url($image).'" title="image" class="popup-lighbox-'.$slider->id.'"><img src="'.base_url($image).'" alt="image"/></a></li>';
					else
						echo '<li style="left: 0px;"><a target="'.$data['thumbnail'][$key].'" href="'.$data['url'][$key].'" title="image"><img src="'.base_url($image).'" alt="image"/></a></li>';
				}else
				{
					if($data['thumbnail'][$key] == 'show')
						echo '<li style="left: 0px;"><a href="'.base_url($image).'" title="image" class="popup-lighbox-'.$slider->id.'"><img src="'.base_url($image).'" alt="image"/></a></li>';
					else
						echo '<li style="left: 0px;"><img src="'.base_url($image).'" alt="image"/></li>';
				}
			}else
			{
				...
				echo '<li style="left: 0px;"><iframe src="'.$data['video_url'][$key].'" width="'.$options->slideWidth.'" height="'.$options->slideHeight.'" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></li>';
			}
		}
	?>
</ul>
